(function () {
	...
	var Game = Invaders.Game = function () {
		...
		this.explosion = new Audio('./assets/sounds/Explosion4.wav');
	};
	...
	Game.prototype.setupAlienFormation = function () {
	  ...
	  for (var i = 0, len = 5 * 11; i < len; i++) {
	    ...
	    switch (gridY) {
	      case 0:
	      case 1:
	        clipRects = Invaders.ALIEN_BOTTOM_ROW;
	        break;
	      case 2:
	      case 3:
	        clipRects = Invaders.ALIEN_MIDDLE_ROW;
	        break;
	      case 4:
	        clipRects = Invaders.ALIEN_TOP_ROW;
	        break;
	    }
	    this.aliens.push(new Invaders.Alien(clipRects, (Invaders.CANVAS_WIDTH / 2 - Invaders.ALIEN_SQUAD_WIDTH / 2) + Invaders.ALIEN_X_MARGIN / 2 + gridX * Invaders.ALIEN_X_MARGIN, Invaders.CANVAS_HEIGHT / 3.25 - gridY * 40));
	    Invaders.alienCount++;
	  }
	};
	...
	Game.prototype.updateAliens = function (dt) {
		if (Invaders.updateAlienLogic) {
	    Invaders.updateAlienLogic = false;
